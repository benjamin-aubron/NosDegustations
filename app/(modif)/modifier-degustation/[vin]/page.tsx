...
export default async function Page({ params }: { params: Promise<{ vin: string }> }) {
  ...
  const formattedSelectedWine: StringifiedData = {
    ...
    pourcentage1: (selectedWine.cepage as Cepage[])?.[0]?.pourcentage != null ? String((selectedWine.cepage as Cepage[])?.[0]?.pourcentage) : "",
    cepage2: (selectedWine.cepage as Cepage[])?.[1]?.cepage || "",
    pourcentage2: (selectedWine.cepage as Cepage[])?.[1]?.pourcentage != null ? String((selectedWine.cepage as Cepage[])?.[1]?.pourcentage) : "",
    cepage3: (selectedWine.cepage as Cepage[])?.[2]?.cepage || "",
    pourcentage3: (selectedWine.cepage as Cepage[])?.[2]?.pourcentage != null ? String((selectedWine.cepage as Cepage[])?.[2]?.pourcentage) : "",
    cepage4: (selectedWine.cepage as Cepage[])?.[3]?.cepage || "",
    pourcentage4: (selectedWine.cepage as Cepage[])?.[3]?.pourcentage != null ? String((selectedWine.cepage as Cepage[])?.[3]?.pourcentage) : "",
